...
export async function postOffer(req: Request, res: Response) {
  try {
    ...
    if (!title || title.length < 4) {
      return res
        .status(400)
        .json({ error: "Le titre est requis (4+ caractères)" });
    }
...
export async function applyToOffer(req: Request, res: Response) {
  try {
    ...
    if (message.length === 0) {
      return res
        .status(400)
        .json({ error: "Le message de candidature est requis" });
    }
    if (message.length > 5000) {
      return res
        .status(400)
        .json({ error: "Le message est trop long (max 5000 caractères)" });
    }
    ...
    if (subject.length > 200) {
      return res
        .status(400)
        .json({ error: "L'objet est trop long (max 200 caractères)" });
    }
    ...
    if (signature && signature.length > 2000) {
      return res
        .status(400)
        .json({ error: "La signature est trop longue (max 2000 caractères)" });
    }
